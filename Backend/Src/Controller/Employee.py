...
class EmployeeController:
    def createEmployee(_name, _phone, _email, _id_category, _status, _createdDate, _updatedDate):
        employee = EmployeeDb(_name.upper(), _phone, _email.upper(
        ), _id_category, _status, _createdDate, _updatedDate)
    ...
    def updateEmployee(id, _name, _phone, _email, _id_category, _status, _updatedDate):
        try:
            EmployeeDb.query.filter_by(id=id).update({'name': _name.upper(), 'phone': _phone, 'email': _email.upper(
            ), 'id_category': _id_category, 'status': _status, 'updatedDate': _updatedDate})
            db.session.commit()
            return True
        except IntegrityError:
            db.session.rollback()
